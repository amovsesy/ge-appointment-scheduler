...
""" % (settings['email_from'], ', '.join(settings['email_to']), avail_apt.strftime('%B %d, %Y %H:%M %p'), current_apt.strftime('%B %d, %Y'))
    try:
        server = smtplib.SMTP('smtp.gmail.com:587')
        server.ehlo()
        server.starttls()
        server.login(settings['email_from'], settings['gmail_pw'])
